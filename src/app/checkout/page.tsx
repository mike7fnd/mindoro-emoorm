        ...
        <main className="flex-grow container mx-auto px-4 pt-0 md:pt-32 pb-24 max-w-[600px]">
          ...
          <div className="space-y-4">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="flex gap-4 p-4 rounded-[25px] border border-black/[0.02]"
              >
        ...
        <div className="flex min-h-screen flex-col bg-white">
          ...
          <main className="flex-grow container mx-auto px-4 pt-0 md:pt-32 pb-24 max-w-[600px]">
            <div className="text-center py-12">
              ...
              <h2 className="text-3xl font-headline font-normal tracking-[-0.03em] mb-2">
                Order Placed!
              </h2>
              ...
              <div className="space-y-6">
                {codOrderIds.map((orderId, idx) => (
                  <div
                    key={orderId}
                    className="bg-[#f8f8f8] rounded-[28px] p-6 border border-black/[0.04]"
                  >
              ...
              <div className="flex flex-col md:flex-row gap-3 justify-center mt-8">
                <Button
                  ...
                  className="rounded-full px-8 py-5 bg-primary text-white font-bold h-12"
                >
                  View Orders
                </Button>
                <Button
                  onClick={() => router.push("/")}
                  variant="outline"
                  className="rounded-full px-8 py-5 h-12"
                >
                  Continue Shopping
                </Button>
              </div>
  ...
  const QRModal = showQRModal && (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-[28px] p-6 w-full max-w-md relative shadow-2xl max-h-[90vh] overflow-y-auto">
        <button
          className="absolute top-4 right-4 text-muted-foreground hover:text-black text-2xl leading-none"
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-xl font-headline font-normal tracking-[-0.03em]">
              Pay via GCash
            </h2>
                ...
                {qrProofUrl ? (
                  <div className="relative">
                    <img
                      src={qrProofUrl}
                      alt="Payment Proof"
                      className="w-full max-h-48 object-contain border-2 border-green-200 rounded-2xl bg-green-50 p-2"
                    />
    ...
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <main className="flex-grow container mx-auto px-0 md:px-6 pt-0 md:pt-32 pb-40 max-w-[1480px]">
        <div className="p-6 md:p-8 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-normal font-headline tracking-[-0.05em] dark:text-white mb-1">
              Checkout
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-6 md:px-8">
          ...
          <div className="md:col-span-2 space-y-8">
            ...
            <div>
              <h2 className="text-xl font-headline font-normal tracking-[-0.03em] mb-2">
                Shipping Address
              </h2>
                ...
                <div className="space-y-2 mb-4">
                  {addressBook.map((addr, idx) => (
                    <div
                      key={idx}
                      className={cn(
                        "p-4 rounded-xl border flex items-center gap-4 cursor-pointer",
              ...
              {showAddAddress && (
                <div className="p-4 rounded-xl border border-primary bg-primary/5 space-y-3 mt-2">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-black/10 rounded-full px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-primary/20"
                  />
                  <div className="flex items-center bg-white border border-black/10 rounded-full focus-within:ring-2 focus-within:ring-primary/20">
                    <span className="pl-4 pr-1 text-sm font-bold text-muted-foreground select-none">
                      +63
                    </span>
                    <input
                      type="tel"
                      ...
                      onChange={(e) => {
                        const v = e.target.value
                          .replace(/\D/g, "")
                          .slice(0, 10);
                        setNewAddress({ ...newAddress, mobile: v });
                      }}
                      maxLength={10}
                      className="flex-1 bg-transparent border-none rounded-r-full py-3 pr-4 text-sm outline-none"
                    />
                  </div>
                  {/* Province locked to Oriental Mindoro */}
                  <div className="w-full bg-white/60 border border-black/10 rounded-full px-4 py-3 text-sm text-muted-foreground cursor-not-allowed">
                    Oriental Mindoro
                  </div>
                  <select
                    value={newAddress.city}
                    onChange={(e) => {
                      const selected = addrCities.find(
                  ...
                  <div className="flex gap-2 mt-2">
                    <Button
                      ...
                      disabled={!newAddress.fullName || !newAddress.city}
              ...
              <div className="space-y-3">
                {[
                  {
                    ...
                    available: fulfillmentAvailability.delivery,
                  },
                  {
                    value: "pickup",
                    label: "Pickup",
                    desc: "You will pick up the order at the shop",
                    available: fulfillmentAvailability.pickup,
                  },
                ]
                ...
                {!fulfillmentAvailability.delivery && (
                  <p className="text-xs text-orange-600 bg-orange-50 rounded-xl p-3">
                    This seller only offers pickup — delivery is not available.
                  </p>
                )}
                {!fulfillmentAvailability.pickup && (
                  <p className="text-xs text-blue-600 bg-blue-50 rounded-xl p-3">
                    This seller only offers delivery — pickup is not available.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="md:col-span-1 space-y-8">
            <div className="rounded-2xl border border-black/10 bg-[#f8f8f8] p-6">
              ...
              <p className="text-sm mb-1">
                {fulfillmentMethod === "pickup"
                  ? "You will pick up the order at the shop."
                  ...
                  <div key={storeGroup.storeId} className="mb-2">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold">
                          {getStoreName(storeGroup.storeId)}
                        </span>
                      </div>
                      <Button
                        size="sm"
                        ...
                        onClick={() =>
                          router.push(`/messages?id=${storeGroup.storeId}`)
                        }
                      >
                        Message Seller
                      </Button>
                    ...
                    <div className="space-y-2">
                      {storeGroup.items.map((item) => (
                        <div
                          key={item.id}
                          className="flex gap-3 p-2 rounded-[12px] bg-white"
                        >
