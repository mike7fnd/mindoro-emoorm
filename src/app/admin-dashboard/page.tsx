...
export default function SellerDashboard() {
  ...
  const { data: allProducts } = useCollection(facilitiesQuery);
  ...
  const stats = [
    { label: "Total sales", value: `₱${totalRevenue.toLocaleString()}`, trend: "+12.5%", positive: true, icon: DollarSign },
    { label: "Active orders", value: recentOrders?.length.toString() || "0", trend: "+4.3%", positive: true, icon: CalendarCheck },
    { label: "Total products", value: allProducts?.length.toString() || "0", trend: "0%", positive: true, icon: TrendingUp },
    { label: "Total buyers", value: allUsers?.length.toString() || "0", trend: "+18.2%", positive: true, icon: Users },
  ];
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 max-w-7xl mx-auto w-full pb-24">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground font-normal">Welcome back. Your store is running smoothly.</p>
            </div>
            <div className="flex gap-3">
              ...
              <Button className="bg-black hover:bg-primary transition-colors rounded-full px-8 h-12 shadow-sm" asChild>
                <Link href="/admin-facilities"><Plus className="mr-2 h-4 w-4" /> Add product</Link>
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-10">
            <Card className="lg:col-span-2 border-none shadow-sm rounded-[25px] overflow-hidden bg-white">
              <CardHeader className="p-8 pb-0">
                <CardTitle className="text-2xl font-headline font-normal tracking-[-0.05em]">Sales growth</CardTitle>
            ...
            <Card className="border-none shadow-sm rounded-[25px] bg-white">
              ...
              <CardContent className="space-y-4 px-8 pb-8">
                <button className="w-full flex items-center gap-3 rounded-full h-14 shadow-sm text-sm border-none bg-[#f8f8f8] hover:bg-muted px-6 transition-all" onClick={() => router.push("/admin-bookings")}>
                  <Search className="h-5 w-5 text-primary" /> View orders
                </button>
                <button className="w-full flex items-center gap-3 rounded-full h-14 shadow-sm text-sm border-none bg-[#f8f8f8] hover:bg-muted px-6 transition-all" onClick={() => router.push("/admin-messages")}>
                  <MessageCircle className="h-5 w-5 text-primary" /> Buyer messages
                </button>
                ...
                <div className="pt-6 mt-4 border-t">
                  <div className="flex items-center gap-4 p-6 bg-primary/5 rounded-[15px]">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium leading-tight">High demand for local produce this weekend. Consider restocking fresh items.</p>
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          <Card className="border-none shadow-sm rounded-[25px] overflow-hidden bg-white">
            <CardHeader className="flex flex-row items-center justify-between p-8">
              <div>
                <CardTitle className="text-2xl font-headline font-normal tracking-[-0.05em]">Recent orders</CardTitle>
                <CardDescription>New incoming orders</CardDescription>
              </div>
                  ...
                  <tbody>
                    {recentOrders?.map((bk) => (
                      <tr key={bk.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="py-6 px-2 text-muted-foreground">{bk.numberOfGuests || 1} items</td>
                        <td className="py-6 px-2 text-right font-black text-lg">₱{bk.totalPrice.toLocaleString()}</td>
                      </tr>
                    ))}
                    {(!recentOrders || recentOrders.length === 0) && (
                      <tr>
                        <td colSpan={4} className="py-20 text-center text-muted-foreground italic">No recent orders found.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
